...
const array = [4, "man", "ali", 8];
function soloStrings(array) {
  return array.filter((article) => typeof article === "string");
}
...
function contieneFalso(array) {
  return array.filter((test) => test === "false");
}
...
function palabrasDeLongitud(array, minLongitud) {
  return array.filter(
    (elem) => elem === "string" && elem.length >= minLongitud
  );
}
...
function numeroDeVocales(string) {
  string.split("'").filter((str) => str.length);
}
